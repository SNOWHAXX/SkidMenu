...
namespace SkidMenu.anticheat.rpc
{
	...
	{
		private static readonly HashSet<SystemTypes> SystemsThatCanBeUpdatedWhenDead = new HashSet<SystemTypes>
		{
			SystemTypes.MedBay,
		...
		private static void ValidateSwitchSystem(PlayerControl player, MessageReader reader, ref bool blockRpc)
		{
			...
			{
				SkidMenu.Log.LogInfo($"Blocked switch update from {player.Data.PlayerName} as lights are not currently sabotaged");
				blockRpc = true;
			}
			...
			{
				SkidMenu.Log.LogInfo($"Blocked switch update from {player.Data.PlayerName} as there is a currently active meeting");
				blockRpc = true;
			}
		}
