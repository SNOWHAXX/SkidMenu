...
namespace SkidMenu.routines
{
	public class PlayerFollowerRoutine : IRoutine
	{
		...
		public override bool Enabled
		{
			...
			set
			{
				...
				if(value && (_enabled != value || !AmAttachedTo(PlayersSection.selectedPlayer)))
				{
                    ...
                    SkidMenu.notifications.Send("Player Follower", $"You are now attached to {following.Data.PlayerName}", 5);
				}
		...
		public override void Run()
		{
			...
			{
				Disable();
                SkidMenu.notifications.Send("Player Follower", "Player Follower was disabled as you left the game.");
				return;
			}
			...
			{
				Disable();
                SkidMenu.notifications.Send("Player Follower", "Player Follower was disabled as the person you attached to left the game.");
				return;
			}
			...
			if(distance > 2)
			{
				SkidMenu.Log.LogInfo($"We drifted too far away from the player we are following, teleporting back to course. Distance: {distance}");
