...
public static class MalumPPMCheats
{
    ...
    public static void KillPlayerPPM()
    {
        if (CheatToggles.killPlayer)
        {
            if (!_killPlayerActive)
            {
                ...
                // Player pick menu made for killing any player using proper host RPC
                PlayerPickMenu.OpenPlayerPickMenu(Utils.GetAllPlayerData(), (Action)(() =>
                {
                    var target = PlayerPickMenu.targetPlayerData.Object;
                    if (target != null) PlayerControl.LocalPlayer.RpcMurderPlayer(target, true);
                }));
    ...
    public static void TelekillPlayerPPM()
    {
        if (CheatToggles.telekillPlayer)
        {
            if (!_telekillPlayerActive)
            {
                ...
                // Telekill using proper TeleMurder coroutine
                PlayerPickMenu.OpenPlayerPickMenu(Utils.GetAllPlayerData(), (Action)(() =>
                {
                    var target = PlayerPickMenu.targetPlayerData.Object;
                    if (target != null)
                        PlayerControl.LocalPlayer.StartCoroutine(PlayersTab.TeleMurder(target).WrapToIl2Cpp());
                }));
    ...
    public static void SpectateDirectly(PlayerControl target)
    {
        if (target == null) return;
        CheatToggles.spectate = true;
