...
public static class MalumCheats
{
    ...
    private static float _petSpamTimer;
    ...
    public static void SpamPetCheat()
    {
        if (!CheatToggles.spamPet || !Utils.isPlayer)
        {
            _petSpamTimer = 0f;
            return;
        }
        ...
        if (lp?.MyPhysics == null || lp.cosmetics?.CurrentPet == null)
        {
            _petSpamTimer = 0f;
            return;
        }

        _petSpamTimer -= Time.deltaTime;
        if (_petSpamTimer > 0f) return;

        float delay = Mathf.Clamp(CheatToggles.spamPetDelay, 0.01f, 1f);
        _petSpamTimer = delay;

        lp.TryPet();
    }

    public static void BreakPetCheat()
    {
        if (!Utils.isPlayer) return;

        var lp = PlayerControl.LocalPlayer;
        if (lp?.cosmetics?.CurrentPet == null) return;

        var pet = lp.cosmetics.CurrentPet;
        pet.SetTargetPlayer(lp);
        pet.SetGettingPet(true, lp.GetTruePosition());
        pet.StartPetAnim();
    }

    private static int _roomEntryLastRoomId = -1;

    public static void RoomEntryCheat()
    {
        if (!CheatToggles.logRoomEntry && !CheatToggles.notifRoomEntry) return;
        try
        {
            if (AmongUsClient.Instance.GameState != InnerNet.InnerNetClient.GameStates.Started)
            {
                _roomEntryLastRoomId = -1;
                return;
            }
            ...
            var localRoom = Utils.GetRoomFromPosition(local.GetTruePosition());
            if (localRoom == null) return;

            int roomId = (int)localRoom.RoomId;
            if (roomId == _roomEntryLastRoomId) return;
            _roomEntryLastRoomId = roomId;

            foreach (var p in PlayerControl.AllPlayerControls)
            {
                if (p == null || p.Data == null || p.Data.IsDead || p.AmOwner || p.PlayerId == local.PlayerId) continue;

                var pRoom = Utils.GetRoomFromPosition(p.GetTruePosition());
                if (pRoom == null || pRoom.RoomId != localRoom.RoomId) continue;

                string roomName = pRoom.RoomId.ToString();
                if (CheatToggles.logRoomEntry)
                    ConsoleUI.Log($"{ConsoleHelper.Fmt(p)} entered <color=#aaddff>{roomName}</color>", "AACCFF");
                if (CheatToggles.notifRoomEntry)
                    SkidMenu.notifications.Send("<color=#aaddff>🚪 Room</color>", $"{NotifHelper.Fmt(p)} entered {roomName}", 3f);
            }
        }
        catch { }
    }
