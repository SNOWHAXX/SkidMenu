...
public static class MalumSabotageCheats
{
    ...
    private const float SpamDelay = 0.11f;
    private static float _spamSabotageAllTimer = 0f;
    private static float _spamFixAllTimer = 0f;
    private static readonly System.Collections.Generic.Dictionary<SystemTypes, float> _spamIndividualTimers = new();
    ...
    {
        bool desired = CheatToggles.unfixableLights || features.FuckGame.Enabled;
        if (desired == _unfixableLights) return;
        if (!_unfixableLights) { CheatToggles.elecSab = false; }
        shipStatus.RpcUpdateSystem(SystemTypes.Electrical, 69);
        _unfixableLights = desired;
    }
    ...
    public static void HandleSpamSabotages()
    {
        if (CheatToggles.spamSabotageAll)
        {
            _spamSabotageAllTimer += UnityEngine.Time.deltaTime;
            if (_spamSabotageAllTimer >= SpamDelay)
            {
                Sabotage.SabotageAll();
                _spamSabotageAllTimer = 0f;
            }
        }
        else
        {
        ...
        if (CheatToggles.spamFixAll)
        {
            _spamFixAllTimer += UnityEngine.Time.deltaTime;
            if (_spamFixAllTimer >= SpamDelay)
            {
        ...
        foreach (var kvp in SabotageTab.SpamIndividual)
        {
            if (!kvp.Value)
            {
                _spamIndividualTimers[kvp.Key] = 0f;
                continue;
            }

            if (!_spamIndividualTimers.ContainsKey(kvp.Key))
                _spamIndividualTimers[kvp.Key] = 0f;

            _spamIndividualTimers[kvp.Key] += UnityEngine.Time.deltaTime;
            if (_spamIndividualTimers[kvp.Key] >= SpamDelay)
            {
                Sabotage.SabotageSystem(kvp.Key);
                _spamIndividualTimers[kvp.Key] = 0f;
            }
        }
    }
